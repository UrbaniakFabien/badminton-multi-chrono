...
$id = $_GET["id"];
$sql = "SELECT reg_joueurs_id,reg_joueurs_nom,reg_joueurs_montant,reg_joueurs_regle
...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    foreach ($row as $key => $value) {
        if ($key != "reg_joueurs_id") {
            $ligne .= "<td style='text-align:center;'>";
            if ($key == "reg_joueurs_regle") {
                $ligne .= "<img  src='../images/" . ($value == 1 ? "regle.png" : "en_attente.png") . "' style='width:25%;' class='saisie_reglement' data-id_reglement='{$row["reg_joueurs_id"]}'/>";
