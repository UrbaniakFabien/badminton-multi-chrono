 ...
 * FUR
 * 10/2018
 * Dédoublonnage si echeanciers concatenés
 * ***************************************************************************** */
    ...
    if ($handle) {
        ...
        while (!feof($handle)) {
            ...
            if (trim($ligne) != "") {
                $num_ligne ++;
                switch ($num_ligne) {
                    case 1 : $titre = $enreg[0];
                        break;
                    case 2 : $lieu_date = $enreg[0];
                    ...
                    default :
                        ...
                        for ($i = 0; $i < count($enreg); $i++) {
                            switch ($i) {
                                //Horaire
                                case 0 : $pos = strpos($enreg[$i], ":");
                                    $horaire = substr($enreg[$i], $pos - 2, 2) . "h" . substr($enreg[$i], $pos + 1, 2);
                                    break;
                                //detail match
                                case 1 :
                                    $tab = analyse_2(ltrim($enreg[$i]));
                                    $spe = $tab[0];
                                    $tableau = $tab[1];
                                    if ($spe == "") {
                                        $spe = $enreg[$i];
                                    ...
                                    break;
                                //numéro de match
                                case 2 : $num_match = $enreg[$i];
                                    break;
                                case 3 : $niveau = $enreg[$i];
                                    $tableau .= " " . $niveau;
                                    break;
//                                case 4 : $tableau .= $enreg[$i] . " VS ";
//                                    break;
//                                case 5 :  $tableau .= $enreg[$i];
//                                    break;
                            }
                        }
        ...
        //nettoyage si echeancier concatené
        //recherche des num_match en double dans l'échéancier en cours
        $sql = "SELECT GROUP_CONCAT(r0.num_match) as lst_num_match 
                FROM (SELECT num_match
                      FROM echeancier 
                      WHERE num_titre = $num_titre 
